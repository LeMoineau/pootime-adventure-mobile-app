...
import PlayerInfosSubHeader from "./elements/PlayerInfosSubHeader";
...
export default function PlayerInfosPage() {
  const route: useRouteType<"PlayerInfos"> = useRoute();
  ...
  const loadUserData = (forceRefresh?: boolean) => {
    if (route.params.uid) {
      get(route.params.uid, forceRefresh).then((res) => {
        if (res) {
          setUserData(res);
        }
      });
                ...
                <>
                  ...
                  >
                    {ShopItems.bodyColors?.items
                      //TODO: remove all old users and try this method
                      // .filter((i) =>
